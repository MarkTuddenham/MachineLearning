...
Matrix::Matrix(size_t t_rows, size_t t_cols, double t_fill, std::string t_name)
    : m_rows(t_rows), m_cols(t_cols), m_name(t_name)
{
...
{
    return (m_data == t_m.m_data) && (m_rows == t_m.get_rows()) && (m_cols == t_m.get_cols());
}
    ...
    else
    {
        ...
        if (m_cols != t_m.m_rows)
        {
            throw std::invalid_argument(
                "Internal dimensions of matrices do not match: " + std::to_string(m_rows) + "x"
                + std::to_string(m_cols) + " and " + std::to_string(t_m.m_rows) + "x"
                + std::to_string(t_m.m_cols));
        }
        else
        {
...
Matrix Matrix::operator*(const Matrix& t_m) const { return this->multiply(t_m); }
...
{
    return apply([](double x) { return (1 / x); });
}
...
Matrix Matrix::operator+(const Matrix& t_m) const { return this->add(t_m); }
...
Matrix Matrix::operator-(const Matrix& t_m) const { return this->subtract(t_m); }
...
size_t Matrix::get_size() const { return m_rows * m_cols; }
...
Matrix& Matrix::reshape(size_t t_rows, size_t t_cols)
{
    ...
    {
        throw std::invalid_argument(
            "Can't reshape matrix with " + std::to_string(get_size()) + " elements to a "
            + std::to_string(t_rows) + "x" + std::to_string(t_cols) + " matrix of "
            + std::to_string(t_rows * t_cols) + " elements.");
    }
...
Matrix Matrix::elementwise(const Matrix& t_m, std::function<double(double, double)> t_f) const
{
    ...
    if (amIScalar || isOtherScalar)
    {
        ...
        for (size_t i = 0; i < res.get_size(); ++i)
        {
            res.m_data[i] = t_f(n, (amIScalar ? t_m.m_data : m_data)[i]);
        }
        return res;
    }
    ...
    {
        throw std::invalid_argument(
            "Matrices are not the same size! " + std::to_string(get_rows()) + "x"
            + std::to_string(get_cols()) + " and " + std::to_string(t_m.get_rows()) + "x"
...
Matrix Matrix::ones(size_t t_rows, size_t t_cols) { return Matrix(t_rows, t_cols, 1); }

Matrix Matrix::zeros(size_t t_rows, size_t t_cols) { return Matrix(t_rows, t_cols, 0); }

std::vector<double> Matrix::flatten(void) const { return m_data; }

size_t Matrix::get_rows() const { return m_rows; }

size_t Matrix::get_cols() const { return m_cols; }

std::string Matrix::get_name() const { return m_name; }
