...
NeuralNetwork::NeuralNetwork(const std::vector<int>& t_layers) : m_layers(t_layers)
{
  ...
  for (size_t i = 1; i < m_layers.size(); ++i)
  {
    ...
    m_weights.push_back(
        Matrix(m_layers[i - 1], m_layers[i]).randomise().set_name("w_" + std::to_string(i)));
    m_biases.push_back(Matrix(1, m_layers[i]).randomise().set_name("b_" + std::to_string(i)));
  }
}

const std::vector<int>& NeuralNetwork::get_layers() const { return m_layers; }
...
void NeuralNetwork::backpropagation(const Matrix& t_inputs, const Matrix& t_targets, double t_eta)
{
  ...
  // loop to compute node deltas and weight error responsibilities from input to last hidden layer
  // (I->H_[N-1])
  ...
  {
    // std::cout << "m_weights[i] = " << m_weights[i].get_rows() << "x" <<
    // m_weights[i].get_cols() << std::endl; std::cout << "w_e[i] = " << w_e[i].get_rows() <<
    // "x" << w_e[i].get_cols() << std::endl;
